...
class RateMyProfApi:
    ...
    def scrape_professors(
        ...
        #f = open("School"+str(school)+".json", "w")
        f = open("RMP_Data_List.json", "a")
        professors = dict()
        ...
        num_pages_div = num_of_pages//10
        print("number of pages divided : "+str(num_pages_div))
        if num_pages_div<1:
            num_pages_div=1
        
        random_list = random.sample(range(1,num_of_pages+1), num_pages_div)

        for i in random_list:  # the loop insert all professor into list
            ...
            for json_professor in json_response["professors"]:
                ...
                page2 = requests.get(
                    ...
                    + str(json_professor["tid"])
                )
                '''
                #str1="http://www.ratemyprofessors.com/ShowRatings.jsp?tid="
                #str2=str(json_professor["tid"])
                #url=str1+str2
                #print(url)
                #self.crawlURL(url)

                line = str(json_professor)
                #print(json.dumps(ast.literal_eval(line)))
                f.write("\n")
                f.write(json.dumps(ast.literal_eval(line)))
                f.write(",")
    ...
    def crawlURL(self,addedURL):
        ...
        for x in range(len(teacherData)):
            if teacherData[x] == 'Level of Difficulty':
                print(str(teacherData[x])+"  "+teacherData[x-1])
                break
...
if __name__ == '__main__':
    ...
    # schools = list(range(1, 6050))
    f = open("RMP_Data_List.json", "w")
    f.write("[")
    f.close()
    #schools = [1205, 186, 1085, 181, 780, 1222, 580, 1255, 1350, 953, 799, 1040]
    #schools = list(range(1, 6050))
    schools  = random.sample(range(1,6050), 1000)
    k = 1
    for school in schools:
        #f = open("School"+str(school)+".txt", "w")
        print("=== Processing School " + str(k) + " ID " + str(school) + " ===")
        k+=1
